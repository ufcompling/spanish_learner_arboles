# e.g., python validate.py path/to/combined/conllu/files --lang es --max-err 0 2> validation_errors.txt
# fix tab formatting issues: sed -E -i '' 's/ \t/\t/g; s/\t /\t/g' filename
# remove 2+ empty lines: sed -E -i '' '/^$/N;/^\n$/D' filename
# remove trailing whitespace: sed -E -i '' 's/[[:space:]]+$//' yourfile.txt
